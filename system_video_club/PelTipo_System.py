...
fl_ruta_file = Path("C:/system_video_club/PeliculaTipo.txt")
fl_ruta_secc = Path("C:/system_video_club/PeliculaTipo.txt")
fl_ruta_json = Path("C:/system_video_club/PeliculaTipo.json")

#estructura del diccionario de usuario

dcc_tipo_pelicula_system ={
    "Id_PeliTipo":"",
    "tipo_de_pelicula":"",
    "costo":"",
    "tiempo_de_alquiler":""
...
def Tipo_Pelicula_Sis():
    ...
    for clave in dcc_tipo_pelicula_system:
        #valor = input("{}:".format(clave));
        #dcc_tipo_pelicula_system[clave]=valor
        if (format(clave) == "Id_PeliTipo"):
            ...
            valor = input("{}:".format(clave))
        dcc_tipo_pelicula_system[clave]=valor
    #dcc_tipo_pelicula_system["Aud_Fech_Ingreso"]= fecha_actual
    #dcc_tipo_pelicula_system["Aud_Pc_Ingreso"] = ip_pc
    ...
    while (True):
        ...
        if (lv_guardar.upper() == "S"):
            ...
            ln_sencc_registro = 0
            ln_sencc_registro = Prc_contrSecuencia("PelTipoSystem")
            with open(fl_ruta_file, "a")as archivo_TipPelicula:
                linea = "PelTipoSystem"+str(ln_sencc_registro)+":";
                for clave, valor in dcc_tipo_pelicula_system.items():
                    if (format(clave) == "Id_PeliTipo"):
                        valor = "Pel.tipo"+ str(ln_sencc_registro)
                    linea += f"{clave} = {valor},";
                linea = linea.rstrip(",")+"\n";
                archivo_TipPelicula.write(linea)
            #actualizar la secuencia de registro
            lv_guardar=Prc_updatesecuencia("PelTipoSystem", ln_sencc_registro);
            if (lv_guardar.upper() == "SI"):
                print("la secuencia de los registros del nuevo tipo de pelicula fue ACTUALIZADO");
            else:
