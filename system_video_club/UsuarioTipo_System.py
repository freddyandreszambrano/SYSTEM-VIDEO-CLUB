...
fl_ruta_file = Path("C:/system_video_club/UsuarioTipoSystem.txt")
fl_ruta_secc = Path("C:/system_video_club/UsuarioTipoSystem.txt")
...
def Tipo_user_sis():
    ...
    for clave in dcc_UsuarioTipo_system:
        ...
        elif (format(clave) == "Tipo_de_cliente"):
            print("-------------")
    ...
    while (True):
        ...
        if (lv_guardar.upper() == "S"):
            ...
            ln_sencc_registro = 0
            ln_sencc_registro = Prc_contrSecuencia("ClientTipoSystem")
            with open(fl_ruta_file, "a")as archivo_tipo_user:
                linea = "ClientTipoSystem"+str(ln_sencc_registro)+":";
                for clave, valor in dcc_UsuarioTipo_system.items():
                    if (format(clave) == "Id_ClienTipo"):
                        valor = "UT" + str(ln_sencc_registro)
                    linea += f"{clave} = {valor},";
                linea = linea.rstrip(",")+"\n";
                archivo_tipo_user.write(linea)
            #actualizar la secuencia de registro
            lv_guardar=Prc_updatesecuencia("ClientTipoSystem", ln_sencc_registro);
